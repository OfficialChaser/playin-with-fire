...
func _ready():
	modulate = GameStats.water_color
	damage = starting_damage
	velocity = Vector2.RIGHT.rotated(rotation + deg_to_rad(randf_range(-1, 1) * GameStats.spread)) * speed
	...
	if body == fire_tiles:
		...
		for x in [-1, 0, 1]:
			for y in [-1, 0, 1]:
				...
				if GameStats.player_health <= float(GameStats.max_hp) / 2:
					damage *= 2
