...
@export var min_lightning : int = 15
...
func get_empty_cells(tiles = fire_tiles, min_tile = Vector2i(-15,-11), max_tile = Vector2i(14,10)) -> Array:
	var possible_cells := []
	
	for x in range(min_tile.x, max_tile.x):
		for y in range(min_tile.y, max_tile.y):
			var coord = Vector2i(x, y)
			if !tiles.get_cell_tile_data(coord) or !tiles.get_cell_tile_data(coord).get_custom_data("boundary"):
