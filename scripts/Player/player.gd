...
func _physics_process(delta):
	if not GameManager.actively_playing or paused:
		hose.spray(false)
...
func get_input_vector() -> Vector2:
	var v = Vector2.ZERO
	var keys = GameStats.keys  # keys order: left, right, up, down
...
func handle_movement(delta):
	...
	if hose.spraying:
		...
		velocity = velocity.move_toward(
			(combined_direction * GameStats.hose_knockback) + jitter, 
			6 * hose_jitter_power * delta
			)
	else:
		velocity = lerp(velocity, input_vector * GameStats.player_move_speed, acc)
